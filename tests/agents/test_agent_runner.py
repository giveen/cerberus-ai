...
@pytest.mark.asyncio
async def test_repeat_empty_tool_calls_inject_reflect_and_schema_template():
    @function_tool(name_override="required_tool")
    def required_tool(url: str) -> str:
        return f"fetched {url}"

    model = FakeModel()
    agent = Agent(name="test", model=model, tools=[required_tool])
    model.add_multiple_turn_outputs(
        [
            [get_function_tool_call("required_tool", json.dumps({}))],
            [get_function_tool_call("required_tool", json.dumps({}))],
            [get_function_tool_call("required_tool", json.dumps({}))],
            [
                get_text_message(
    ...
    assert result.final_output.endswith('{"status":"done"}')
    assert isinstance(result.input, list)
    assert any(
        isinstance(item, dict)
        and any(
            "You drafted arguments in <think> but failed to include them in the JSON"
    ...
    assert any(
        isinstance(item, dict)
        and any(
            "Use this exact template for required_tool" in str(block.get("text", ""))
            for block in item.get("content", [])
            if isinstance(block, dict)
        )
        for item in result.input
    ...
    assert not any(
        isinstance(item, dict)
        and any(
            "STRATEGY SHIFT REQUIRED" in str(block.get("text", ""))
            for block in item.get("content", [])
            if isinstance(block, dict)
        )
        for item in result.input
...
async def test_handoffs():
    ...
    assert len(result.to_input_list()) == 8, (
        "should have 8 inputs: orig input, tool call, tool result, message, handoff state, "
        "handoff call, handoff result, and done message"
    )
...
async def test_structured_output():
    ...
    assert len(result.to_input_list()) == 11, (
        ...
        "handoff state inject, handoff output, tool call, tool call result, final output message"
    )
