...
def test_message_to_output_items_with_legacy_function_call_shape(converter):
    """Legacy assistant.function_call payloads should still map to function_call output items."""
    legacy_msg = cast(
        ...
        SimpleNamespace(
            ...
            audio=None,
            tool_calls=None,
            function_call={"name": "handoff_to_recon", "arguments": '{"target":"x"}'},
        ),
    )

    items = converter.message_to_output_items(legacy_msg)
    assert len(items) == 1
    fn_call_item = cast(ResponseFunctionToolCall, items[0])
    assert fn_call_item.name == "handoff_to_recon"
    assert fn_call_item.arguments == '{"target":"x"}'
    assert fn_call_item.call_id.startswith("call_")


def test_message_to_output_items_with_dict_tool_call_shape(converter):
    """Dict-shaped tool_calls emitted by non-OpenAI proxies should be normalized."""
    dict_tool_msg = cast(
        ChatCompletionMessage,
        SimpleNamespace(
            role="assistant",
    ...
    items = converter.message_to_output_items(dict_tool_msg)
    assert len(items) == 1
    fn_call_item = cast(ResponseFunctionToolCall, items[0])
    assert fn_call_item.call_id == "call-proxy-1"
    assert fn_call_item.name == "handoff_to_exploit"
    assert fn_call_item.arguments == '{"target":"10.0.0.5"}'
