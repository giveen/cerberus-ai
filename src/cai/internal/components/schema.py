...
class CerebroSchemaModel(BaseModel):
...
class CredentialLoot(CerebroSchemaModel):
...
class FileLoot(CerebroSchemaModel):
...
class HashLoot(CerebroSchemaModel):
...
class ExtractedLoot(CerebroSchemaModel):
...
class VulnerabilityDetails(CerebroSchemaModel):
...
class CerebroFinding(CerebroSchemaModel):
...
class CerebroLogicNode(CerebroSchemaModel):
...
class ExecutionTelemetry(CerebroSchemaModel):
...
class ToolRequest(CerebroSchemaModel):
...
class MissionScope(CerebroSchemaModel):
...
class EngineHealth(CerebroSchemaModel):
...
class RamSegmentHeartbeat(CerebroSchemaModel):
...
class SystemHeartbeat(CerebroSchemaModel):
...
class JsonlBatch(CerebroSchemaModel):
    """High-velocity container for batched JSONL persistence."""

    records: List[Dict[str, Any]] = Field(default_factory=list)

    @classmethod
    def from_models(cls, models: Iterable[CerebroSchemaModel]) -> "JsonlBatch":
