...
from cai.internal.components.schema import CommandInvocation, ExecutionTelemetry, ToolResult
from cai.internal.registry.commands import CerebroCommandRegistry
from cai.internal.util.help_formatter import generate_command_help
...
from cai.util import get_system_telemetry
...
class CommandDispatcher:
    ...
    def __init__(self) -> None:
        self._registry = CerebroCommandRegistry()
    ...
    def _validate_parallel_capacity(self, envelope: CommandInvocation) -> str:
        ...
        config = self._load_cerebro_config()
    ...
    @staticmethod
    def _load_cerebro_config() -> Any:
        module = importlib.import_module("cai.util.config")
        getter = getattr(module, "get_cerebro_config")
        return getter()
    ...
    def _run_exit_hooks() -> None:
        ...
        try:
            ...
            bus = CerebroMemoryBus.get_instance(workspace_root=workspace)
            bus.commit()
        except Exception:
            pass

        try:
            from cai.internal.components.transfer import cost_engine
    ...
    def _flush_chpe_before_command() -> None:
        try:
            from cai.memory.storage import CerebroStorageHandler

            storage = CerebroStorageHandler.get_instance(
