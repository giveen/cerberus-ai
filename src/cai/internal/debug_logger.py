"""Local-only debug logger for CAI runtime internals.
...
def _workspace_root() -> Path:
    explicit = os.getenv("CEREBRO_WORKSPACE_ACTIVE_ROOT") or os.getenv("WORKSPACE_ROOT")
    if explicit:
...
def get_debug_logger() -> DebugLogger:
    ...
    if _LOGGER is None:
        with _LOGGER_LOCK:
            if _LOGGER is None:
                log_path = _workspace_root() / ".cai" / "runtime_debug.log"
