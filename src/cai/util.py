...
def load_prompt_template(template_path: str) -> str:
    ...
    try:
        resource = importlib.resources.files("cai") / template_path
        if resource.is_file():
...
class CostTracker:
    ...
    def _pricing_files(self) -> list[Path]:
        ...
        override = os.getenv("CEREBRO_PRICING_JSON", "").strip()
        if override:
    ...
    def _read_remote_pricing(self, model_name: str) -> tuple[float, float] | None:
        if os.getenv("CEREBRO_ENABLE_REMOTE_PRICING", "").strip().lower() in {"0", "false", "no", "off"}:
            return None

        url = os.getenv(
            "CEREBRO_LITELLM_PRICING_URL",
    ...
    def check_price_limit(self, estimated_cost: float) -> None:
        ...
        raw_limit = os.getenv("CEREBRO_MAX_USD", "").strip()
        if not raw_limit:
            return
        try:
...
def get_model_input_tokens(model: Any) -> int:
    ...
    if any(tag in model_name for tag in ("gpt-4o", "gpt-4.1", "claude-3", "o3", "o4")):
        return 128000
    return int(os.getenv("CEREBRO_DEFAULT_CONTEXT_WINDOW", "32768"))
...
def cli_print_tool_output(
    ...
    **_: Any,
    ...
    - `CEREBRO_STREAM=true`: suppress repeated command keys for the whole session.
    - `CEREBRO_STREAM=false`: suppress repeated command keys within 0.5 seconds.
    """
    ...
    streaming_enabled = os.getenv("CEREBRO_STREAM", "false").strip().lower() == "true"
...
def start_claude_thinking_if_applicable(model_name: str, agent_name: str, counter: int) -> dict[str, Any] | None:
    ...
    if os.getenv("CEREBRO_STREAM", "false").strip().lower() != "true":
        return None
