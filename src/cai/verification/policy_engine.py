...
_PATH_KEYWORDS = ("path", "file", "dir", "workspace", "output", "input", "destination", "source")
...
class PolicyEngine:
    ...
    def __init__(self, *, workspace_dir: str | None = None, project_id: str | None = None) -> None:
        workspace = (workspace_dir or os.getenv("CEREBRO_WORKSPACE_ACTIVE_ROOT") or os.getcwd()).strip()
        self.workspace_root = Path(workspace).resolve()
        self.project_id = self._normalize_project_id(project_id or os.getenv("CEREBRO_PROJECT_ID") or "")
