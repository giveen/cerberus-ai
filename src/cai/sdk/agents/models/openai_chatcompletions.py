...
from cai.internal.debug_logger import get_debug_logger
...
from cai.util.config import get_effective_api_base, get_effective_api_key
...
if os.getenv("CEREBRO_MODEL") == "o3-mini" or os.getenv("CEREBRO_MODEL") == "gemini-1.5-pro":
...
def _normalize_litellm_routing_kwargs(kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    needs_local_openai = (
        bool(os.getenv("CEREBRO_API_BASE"))
...
class ContextCompactedError(Exception):
    """Raised inside get_response/stream_response when a CEREBRO_SUPPORT_INTERVAL-based
...
class OpenAIChatCompletionsModel(Model):
    ...
    async def get_response(
        ...
        with generation_span(
            ...
            os.environ['CEREBRO_CONTEXT_USAGE'] = str(context_usage)
            ...
            if should_display_message:
                ...
                previous_stream_setting = os.environ.get("CEREBRO_STREAM", "false")
                os.environ["CEREBRO_STREAM"] = "false"  # Force non-streaming mode for markdown parsing
                ...
                os.environ["CEREBRO_STREAM"] = previous_stream_setting
            ...
            if hasattr(assistant_msg, "tool_calls") and assistant_msg.tool_calls:
                ...
                if "openai/gemini" in os.getenv("CEREBRO_MODEL"):
    ...
    async def stream_response(
        ...
        try:
            ...
            should_show_rich_stream = (
                os.getenv("CEREBRO_STREAM", "false").lower() == "true"
                ...
                try:
                    async for chunk in stream:
                        ...
                        if content:
                            ...
                            # Update streaming display if enabled - ALWAYS respect CEREBRO_STREAM setting
    ...
    async def _fetch_response(
        ...
        if "cerebro" in model_str and "cerebro1.5" not in model_str:  # NOTE: exclude cerebro1.5
            ...
            kwargs["api_key"] = get_effective_api_key(default="REDACTED_CEREBRO_KEY")
        elif "/" in model_str:
        ...
        uses_local_or_custom_endpoint = (
            ...
            or bool(os.getenv("CEREBRO_API_BASE"))
        ...
        preflight_payload = {
            ...
            "target_url": str(kwargs.get("api_base") or os.getenv("CEREBRO_API_BASE", "http://localhost:8000/v1")),
    ...
    async def _fetch_response_litellm_openai(
        ...
        timeout_seconds = _parse_timeout_seconds(os.getenv("CEREBRO_LLM_TIMEOUT_SECONDS"))

        def _audit_connection_failure(exc: Exception) -> None:
            workspace = os.getenv("CIR_WORKSPACE") or os.getenv("CEREBRO_WORKSPACE_ROOT") or os.getcwd()
            try:
                from cai.repl.ui.logging import get_cerebro_logger

                get_cerebro_logger().audit(
        ...
        try:
            ...
            raise UserError(
                "LLM request rejected by provider. Verify model routing and CEREBRO_API_BASE configuration."
            ) from exc
            ...
            raise UserError(
                "LLM endpoint returned 404. Set CEREBRO_API_BASE to your active local endpoint "
    ...
    async def _auto_compact_if_needed(self, estimated_tokens: int, input: str | list[TResponseInputItem], system_instructions: str | None) -> tuple[str | list[TResponseInputItem], str | None, bool]:
        ...
        if os.getenv("CEREBRO_AUTO_COMPACT", "true").lower() == "false":
            return input, system_instructions, False

        # --- CEREBRO_SUPPORT_INTERVAL count-based trigger ---
        ...
        _support_model = os.getenv("CEREBRO_SUPPORT_MODEL")
        _support_interval_raw = os.getenv("CEREBRO_SUPPORT_INTERVAL")
        ...
        max_tokens = self._get_model_max_tokens(str(self.model))
        try:
            threshold_percent = float(os.getenv("CEREBRO_AUTO_COMPACT_THRESHOLD", "0.8"))
        ...
        try:
            ...
            if summary:
                ...
                os.environ['CEREBRO_CONTEXT_USAGE'] = str(new_context_usage)
    ...
    def _get_client(self) -> AsyncOpenAI:
        if self._client is None:
            ...
            base_url = os.getenv("CEREBRO_API_BASE", "http://localhost:8000/v1")
    ...
    def items_to_messages(
        ...
        for item in items:
            ...
            # 5) function call output => tool message
                ...
                is_streaming_enabled = os.environ.get("CEREBRO_STREAM", "false").lower() == "true"
