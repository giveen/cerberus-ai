...
try:
    from cai.repl.commands.agent import AgentRegistry, AgentConfig
except ImportError:
    AgentRegistry = None
    AgentConfig = None
...
class AgentDiscoveryEngine:
    ...
    async def _log_discovery_event(
        ...
        if not self._logger and get_cerebro_logger:
...
def initialize_discovery(registry: Optional[Any] = None) -> None:
    ...
    if get_cerebro_logger:
        try:
            _DISCOVERY_ENGINE.set_logger(get_cerebro_logger())
        except Exception:
            pass
