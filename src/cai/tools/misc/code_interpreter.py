...
class CerebroInterpreter:
    ...
    async def _run_in_ephemeral_docker(
        ...
        *,
        ...
        image = os.getenv("CEREBRO_INTERPRETER_DOCKER_IMAGE", "python:3.12-slim")
        runtime_root = (self._workspace / ".cai" / "interpreter_runtime").resolve()
    ...
    def _is_high_isolation_mode(self) -> bool:
        env_flag = os.getenv("CEREBRO_INTERPRETER_HIGH_ISOLATION", "").strip().lower()
        if env_flag in {"1", "true", "yes", "on"}:
            return True
        try:
            resolved, _tier = CONFIG_STORE.resolve("CEREBRO_INTERPRETER_HIGH_ISOLATION")
    ...
    def _audit_execution(self, payload: Mapping[str, Any]) -> None:
        audit_path = (self._workspace / ".cai" / "audit" / "python_interpreter.jsonl").resolve()
