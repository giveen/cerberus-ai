...
def _fallback_workspace_base() -> Path:
    ...
    return (cache_root / "cai" / "workspaces").resolve()
...
class ProjectSpace:
    ...
    def from_environment(cls, env: Mapping[str, str] | None = None) -> ProjectSpace:
        ...
        explicit_active = source.get("CEREBRO_WORKSPACE_ACTIVE_ROOT")
        if explicit_active:
            active = Path(explicit_active).expanduser().resolve()
            return cls(root_base=active.parent, session_id=active.name)

        explicit_base = source.get("CEREBRO_WORKSPACE_DIR")
        explicit_name = source.get("CEREBRO_WORKSPACE")
        ...
        if _is_valid_workspace_name(explicit_name):
            ...
            _warn(f"Invalid CEREBRO_WORKSPACE '{explicit_name}'. Using auto-generated run directory instead.")
    ...
    def initialize(self) -> Path:
        try:
            ...
            if isinstance(exc, PermissionError):
                ...
                os.environ["CEREBRO_WORKSPACE_ACTIVE_ROOT"] = str(fallback_root)
                self._initialized = True
                return self.session_root
...
class CerebroWorkspaceTool:
    ...
    AUX_READ_ZONES = {"logs", "artifacts", "findings", "reports", "shared", "private", ".cai"}
    FORENSIC_FILE = ".cai/workspace/forensic_labels.jsonl"
    RETENTION_FILE = ".cai/workspace/retention_stage.json"
    LOCK_FILE = ".cai/workspace/ops.lock"
    ...
    def _ensure_layout(self) -> None:
        ...
        (self._root / ".cai" / "workspace").mkdir(parents=True, exist_ok=True)
    ...
    async def get_summary(self) -> Dict[str, Any]:
        ...
        try:
            def _scan() -> WorkspaceSummary:
                ...
                case_name = os.getenv("CEREBRO_WORKSPACE") or self._root.name
...
def _get_workspace_dir() -> str:
    workspace_active_root = os.getenv("CEREBRO_WORKSPACE_ACTIVE_ROOT")
    ...
    workspace_name = os.getenv("CEREBRO_WORKSPACE")
    workspace_base = os.getenv("CEREBRO_WORKSPACE_DIR")
...
def _get_container_workspace_path() -> str:
    ...
    if workspace_name:
        _warn(f"Invalid CEREBRO_WORKSPACE '{workspace_name}' for container path. Using '/'.")
