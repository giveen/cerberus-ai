...
class CerebroExecTool:
    ...
    async def _execute_async(
        ...
        *,
        ...
        tmp_root = (self._workspace / ".cai" / "tmp").resolve()
        tmp_root.mkdir(parents=True, exist_ok=True)
        scratch = Path(tempfile.mkdtemp(prefix=f"cai_exec_{execution_id}_", dir=str(tmp_root)))
    ...
    def _is_unrestricted_mode(self) -> bool:
        for key in ("CEREBRO_EXPERT_MODE", "CEREBRO_UNRESTRICTED_MODE"):
