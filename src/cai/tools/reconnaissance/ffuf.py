...
from cai.utils.streamer import StreamingContext, capture_streaming_context, run_streaming_subprocess
...
class CerebroFfufTool:
    ...
    def __init__(self) -> None:
        ...
        self._audit_log = (self._workspace / ".cai" / "audit" / "ffuf_jobs.jsonl").resolve()
    ...
    async def _calibrate_false_positive(
        ...
        token = f"cai-cal-{secrets.token_hex(6)}"
    ...
    def _resolve_politeness_rate(self, explicit: Optional[Union[int, float, str]]) -> float:
        if explicit is not None and str(explicit).strip():
            try:
                ...
                pass
        for key in ("CEREBRO_FFUF_POLITENESS_RPS", "CEREBRO_DISCOVERY_RPS", "CEREBRO_SAFE_RPS"):
