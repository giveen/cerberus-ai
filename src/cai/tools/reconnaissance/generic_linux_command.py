...
from cai.tools.common import ACTIVE_SESSIONS, _resolve_session_id, get_session_output, list_shell_sessions, run_command, run_command_async, terminate_session
...
_DEFAULT_TIMEOUT_SECONDS = max(30, int(os.getenv("CEREBRO_COMMAND_TIMEOUT_SECONDS", "600")))
_MAX_TIMEOUT_SECONDS = max(_DEFAULT_TIMEOUT_SECONDS, int(os.getenv("CEREBRO_COMMAND_TIMEOUT_MAX", "3600")))
...
def _format_environment_info() -> str:
    ...
    try:
        from cai.cli import ctf_global  # type: ignore[import-untyped]
    ...
    active_container = os.getenv("CEREBRO_ACTIVE_CONTAINER", "").strip() or os.getenv("CAI_ACTIVE_CONTAINER", "").strip()
