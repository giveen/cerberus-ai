...
from cai.repl.commands.config import get_env_var_value
...
class CerebroShodanTool:
    ...
    def __init__(self) -> None:
        ...
        self._audit_log = (self._workspace / ".cai" / "audit" / "shodan_queries.jsonl").resolve()
        self._quota_state = (self._workspace / ".cai" / "audit" / "shodan_quota_state.json").resolve()
    ...
    async def _credit_guardrail(self, *, api: Any, planned_queries: int) -> Optional[SemanticError]:
        quota_raw = (os.getenv("CEREBRO_SHODAN_QUERY_QUOTA", "") or "").strip()
