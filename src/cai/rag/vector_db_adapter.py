...
def _active_workspace_scope() -> Dict[str, Optional[str]]:
    ...
    workspace_root = (
        os.getenv("CEREBRO_WORKSPACE_ACTIVE_ROOT")
    ...
    workspace_id = os.getenv("CEREBRO_WORKSPACE") or os.path.basename(workspace_root.rstrip("/"))
    session_id = os.getenv("CEREBRO_SESSION_ID") or os.getenv("SESSION_ID")
    return {
...
def get_vector_db_adapter(name: Optional[str] = None, **kwargs) -> VectorDBAdapter:
    """Factory to get an adapter by name or environment `CEREBRO_VECTOR_DB`.

    Behavior:
    - If the caller provides `name` or `CEREBRO_VECTOR_DB` env is set, that value
      is used.
    ...
    """
    env_value = os.getenv("CEREBRO_VECTOR_DB")
    ...
    if not source:
        ...
        elif os.getenv("QDRANT_URL") or os.getenv("CEREBRO_QDRANT_URL"):
            source = "qdrant"
        else:
...
class QdrantAdapter(VectorDBAdapter):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        check if `QDRANT_URL` or `CEREBRO_QDRANT_URL` is provided.
        """
        try:
            self._ensure_client()
        ...
        qdrant_url = os.getenv("QDRANT_URL") or os.getenv("CEREBRO_QDRANT_URL")
        if qdrant_url:
            try:
...
class MemPalaceAdapter(VectorDBAdapter):
    ...
    def __init__(self, palace_path: Optional[str] = None, config: Optional[Dict[str, Any]] = None, embeddings_provider: Optional[Any] = None):
        ...
        self.palace_path = (
            ...
            or os.getenv("CEREBRO_MEMPALACE_PATH", "~/.mempalace/palace")
        )
...
class LocalFallbackAdapter(VectorDBAdapter):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, embeddings_provider: Optional[Any] = None):
        ...
        env_use = os.getenv("CEREBRO_USE_FAISS", "").strip()
        ...
        self._persist_dir = os.path.expanduser(os.getenv("CEREBRO_LOCAL_PERSIST_DIR", "~/.cai/memory/local/"))
        try:
    ...
    def add_points(self, id_point: Any, collection_name: str, texts: List[str], metadata: List[dict]) -> bool:
        ...
        try:
            session = os.getenv("CEREBRO_SESSION_ID") or os.getenv("SESSION_ID")
        except Exception:
            session = None
