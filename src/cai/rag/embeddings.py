...
when `CEREBRO_EMBEDDINGS_PROVIDER` is set to `openai` and the runtime has
...
class CUDAEmbeddingsProvider(EmbeddingsProvider):
    ...
    Set ``CEREBRO_CUDA_MODEL`` env var to choose the ST model
    (default: ``BAAI/bge-m3``).
    """
...
def get_embeddings_provider(name: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> EmbeddingsProvider:
    ...
    `CEREBRO_EMBEDDINGS_PROVIDER` is consulted. When unset, prefer OpenAI if
    an API key is present; otherwise fall back to the local deterministic
    provider.
    """
    chosen = (name or os.getenv("CEREBRO_EMBEDDINGS_PROVIDER") or "").lower()
    if not chosen:
