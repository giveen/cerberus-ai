...
def _default_triplestore_path() -> str:
    return os.environ.get("CEREBRO_TRIPLESTORE_PATH") or os.path.join(os.getcwd(), ".cai", "triplestore.db")
...
def get_global_triplestore(db_path: Optional[str] = None, pragmas: Optional[Dict[str, str]] = None) -> TripleStore:
    ...
    `CEREBRO_TRIPLESTORE_PATH` env var, falling back to `.cai/triplestore.db`.
