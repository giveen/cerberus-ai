...
def _is_local_provider_configured() -> bool:
    ...
    model_value = os.getenv("CEREBRO_MODEL", "").strip().lower()
    local_prefixes = (
        "ollama",
        "litellm",
        "llama",
...
def should_suppress_openai_api_key_warning() -> bool:
    ...
    audit_mode = os.getenv("AUDIT_MODE", os.getenv("CEREBRO_AUDIT_MODE", "TRANSPARENT"))
...
def get_effective_api_base(default: str = "http://localhost:8000/v1") -> str:
    """Return API base honoring the CEREBRO-prefixed variable."""
    return (
        os.getenv("CEREBRO_API_BASE")
        or default
    )


def get_effective_api_key(default: str = "sk-cerebro-1234567890") -> str:
    """Return API key honoring CEREBRO key first, then OpenAI fallback."""
    return (
...
class CerebroConfig(BaseSettings):
    ...
    workspace_root: Path = Field(
        ...
        validation_alias=AliasChoices("CIR_WORKSPACE", "CEREBRO_WORKSPACE_ROOT"),
    )
    total_system_ram_gb: float = Field(
        default_factory=_detect_total_system_ram_gb,
        validation_alias=AliasChoices("CEREBRO_TOTAL_SYSTEM_RAM_GB", "CEREBRO_RAM_TOTAL_GB"),
    )
    ...
    use_gpu_acceleration: bool = Field(
        default=True,
        validation_alias=AliasChoices("CEREBRO_USE_GPU_ACCELERATION", "USE_GPU_ACCELERATION"),
    )
    cuda_device: str = Field(default="cuda:0", validation_alias=AliasChoices("CEREBRO_CUDA_DEVICE", "CUDA_DEVICE"))
    cuda_visible_devices: str = Field(
        default=os.getenv("CUDA_VISIBLE_DEVICES", "0"),
        validation_alias=AliasChoices("CUDA_VISIBLE_DEVICES", "CEREBRO_CUDA_VISIBLE_DEVICES"),
    )
    ...
    log_level: str = Field(default="INFO", validation_alias=AliasChoices("CEREBRO_LOG_LEVEL", "LOG_LEVEL"))

    @model_validator(mode="after")
    def validate_allocations(self) -> "CerebroConfig":
    ...
    def paths(self) -> PathHierarchy:
        ...
        return PathHierarchy(
            ...
            audit_dir=(root / ".cai" / "audit").resolve(),
    ...
    def as_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "CUDA_VISIBLE_DEVICES": self.cuda_visible_devices,
            "CEREBRO_USE_GPU_ACCELERATION": str(self.use_gpu_acceleration).lower(),
            "CEREBRO_TOTAL_SYSTEM_RAM_GB": str(self.total_system_ram_gb),
            "CEREBRO_LOG_LEVEL": self.log_level,
        }


@lru_cache(maxsize=1)
def get_cerebro_config() -> CerebroConfig:
    """Return the process-wide immutable Cerebro configuration singleton."""
