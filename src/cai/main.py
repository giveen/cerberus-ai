...
from cai.tools.all_tools import get_tool_registry
from cai.tools.reconnaissance.filesystem import PathGuard
from cai.utils.streamer import streaming_runtime, terminate_registered_processes
from cai.util.config import should_suppress_openai_api_key_warning
from cai.verification.policy_engine import PolicyEngine
...
def resolve_headless_workspaces_root(workspace_dir: str | Path | None = None) -> Path:
    if workspace_dir is None:
        candidate = Path(os.getenv("CEREBRO_WORKSPACE_ROOT") or Path.cwd()).resolve()
    else:
...
def _temporary_workspace_environment(
    ...
    os.environ["CEREBRO_WORKSPACE_ACTIVE_ROOT"] = str(project_root)
    os.environ["CEREBRO_WORKSPACE_DIR"] = str(workspaces_root)
    os.environ["CEREBRO_WORKSPACE"] = project_root.name
...
async def _invoke_streamable_tool(tool_name: str, arguments: dict[str, Any], log_emitter: Any) -> tuple[Any, bool]:
    ...
    *,
    ...
    try:
        with streaming_runtime(session_id=session_id, callback=lambda channel, message: _emit_headless_log(log_emitter, channel=channel, message=message)):
            output, handled = await _invoke_streamable_tool(tool_name, arguments, log_emitter)
            if not handled:
...
async def terminate_action(session_id: str) -> dict[str, Any]:
    ...
    result = await terminate_registered_processes(normalized)
...
class CerebroMissionControl:
    ...
    async def _initialize_hardware(self) -> HardwareProfile:
        ...
        strict = os.getenv("CEREBRO_PREFLIGHT_STRICT", "true").lower() != "false"
        if self.open_mode:
            strict = False
    ...
    async def _initialize_memory(self) -> MemorySuite:
        ...
        vector_provider = await asyncio.to_thread(
            ...
            "cuda",
            {
                "model_name": os.getenv("CEREBRO_CUDA_MODEL", "BAAI/bge-m3"),
                "batch_size": 256,
                "normalize": True,
            },
        )
    ...
    async def _run_dry_run_validation(self) -> None:
        ...
        tool_name = os.getenv("CEREBRO_DRY_RUN_TOOL", "python3")
    ...
    async def _load_scope(self) -> MissionScope:
        ...
        env_scope = os.getenv("CEREBRO_SCOPE_JSON", "").strip()
        env_scope_source = os.getenv("CEREBRO_SCOPE_SOURCE", "").strip()
        ...
        if env_scope:
            payload = json.loads(env_scope)
            return MissionScope(source="env:CEREBRO_SCOPE_JSON", payload=payload)
        raise PreFlightError(
            "Mission scope missing: provide scope.json in the workspace root or set CEREBRO_SCOPE_JSON"
        )
...
def _resolve_scope_into_env(scope_path: str, workspace_root: Path) -> None:
    ...
    os.environ["CEREBRO_SCOPE_JSON"] = json.dumps(payload, ensure_ascii=True)
    os.environ["CEREBRO_SCOPE_SOURCE"] = f"cli:{resolved}"
...
def main(*, register_main_thread_signals: bool = True) -> int:
    ...
    if args.version:
        _emit_headless_message(_read_cai_version())
        return 0
    ...
    if args.init_only and not args.scope and not os.getenv("CEREBRO_SCOPE_JSON") and not (workspace_root / "scope.json").exists():
        os.environ["CEREBRO_SCOPE_JSON"] = json.dumps(
            {
                "targets": [],
                "authorized_cidrs": [],
