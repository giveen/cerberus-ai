...
from cai.sdk.agents.models.openai_chatcompletions import get_all_agent_histories, get_agent_message_history
...
def _get_agent_manager() -> Any:
    from cai.sdk.agents.simple_agent_manager import AGENT_MANAGER as agent_manager

    return agent_manager


def _get_history_runtime_state() -> Tuple[Dict[Any, Any], Dict[str, List[Dict[str, Any]]]]:
    from cai.sdk.agents.models import openai_chatcompletions as chat_models
    ...
    def _restore_agent_context(self, *, agents: AgentContextPayload, merge: bool) -> None:
        ...
        if agents.parallel_configs:
            ...
            if len(PARALLEL_CONFIGS) >= 2:
                os.environ["CEREBRO_PARALLEL"] = str(len(PARALLEL_CONFIGS))
                os.environ["CEREBRO_PARALLEL_AGENTS"] = ",".join(c.agent_name for c in PARALLEL_CONFIGS)
