...
from cai.memory.storage import EvidenceRecord, WorkspaceJSONStore
from cai.repl.commands.base import FrameworkCommand, register_command
from cai.repl.commands.cost import BudgetPolicy, USAGE_TRACKER, UsageRecord
...
class DataReconciler:
    ...
    def __init__(self, *, memory_manager: MemoryManager, workspace_root: Path) -> None:
        ...
        self._audit_path = self._workspace_root / ".cai" / "audit" / "merge_audit.jsonl"
    ...
    async def _load_source_envelope(self, source_root: Path) -> ImportedSessionEnvelope:
        source_file = source_root / ".cai" / "session" / "latest.session.json"
    ...
    async def _load_source_findings(self, source_root: Path) -> List[EvidenceRecord]:
        source_file = source_root / ".cai" / "memory" / "evidence.jsonl"
        if not source_file.exists():
            return []
    ...
    async def _write_cost_rollup_atomically(
        ...
        *,
        ...
        target = self._workspace_root / ".cai" / "session" / "merged_cost_ledger.json"
        payload = {
...
class MergeCommand(FrameworkCommand):
    ...
    async def execute(self, args: List[str]) -> bool:
        ...
        console.print(
            Panel(
                f"Merge complete. Audit log written to {self._workspace_root / '.cai' / 'audit' / 'merge_audit.jsonl'}",
