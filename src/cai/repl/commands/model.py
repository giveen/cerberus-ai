...
from cai.repl.commands.base import Command, CommandError, register_command
...
LITELLM_MODEL_CATALOG_URL = os.getenv("CEREBRO_LITELLM_MODEL_CATALOG_URL", "").strip()
...
class LocalModelProvider(ModelProviderInterface):
    ...
    def _discover_vllm_models() -> List[str]:
        explicit = os.getenv("CEREBRO_VLLM_MODELS", "")
...
class ModelOrchestrator:
    ...
    def __init__(self, *, memory: MemoryManager, workspace_root: Path) -> None:
        ...
        self._selection_path = self._workspace_root / ".cai" / "session" / "model_selection.json"
        self._audit_path = self._workspace_root / ".cai" / "audit" / "model_actions.jsonl"
    ...
    def current_selection(self) -> ModelSelectionState:
        env_model = os.getenv("CEREBRO_MODEL", "cerebro1")
    ...
    async def capability_audit(self, model_id: str) -> List[str]:
        ...
        ctx_env = os.getenv("CEREBRO_CONTEXT_WINDOW") or os.getenv("CEREBRO_MAX_CONTEXT")
    ...
    def _apply_runtime_selection(self, state: ModelSelectionState) -> None:
        os.environ["CEREBRO_MODEL"] = state.selected_model
        os.environ["CEREBRO_MODEL_PROVIDER"] = state.provider
    ...
    def _apply_runtime_parameters(params: Mapping[str, Any]) -> None:
        if "temperature" in params:
            os.environ["CEREBRO_TEMP"] = str(params["temperature"])
        if "context_window" in params:
            os.environ["CEREBRO_CONTEXT_WINDOW"] = str(params["context_window"])
        if "max_tokens" in params:
            os.environ["CEREBRO_MAX_TOKENS"] = str(params["max_tokens"])
...
async def _load_litellm_catalog() -> Dict[str, Dict[str, Any]]:
    if os.getenv("CEREBRO_ENABLE_REMOTE_MODEL_CATALOG", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return {}
...
class ModelCommand(Command):
    ...
    def help(self) -> str:
        return (
            ...
            "\n"
            "Model changes are persisted in workspace .cai/session/model_selection.json"
        )
    ...
    def _handle_legacy_selection(self, args: List[str]) -> bool:
        ...
        os.environ["CEREBRO_MODEL"] = selected_model
        os.environ["CEREBRO_MODEL_PROVIDER"] = self._orchestrator._guess_provider(selected_model)  # pylint: disable=protected-access
        console.print(
            Panel(
    ...
    def _render_legacy_status_table(self) -> None:
        current_model = os.getenv("CEREBRO_MODEL", "")
