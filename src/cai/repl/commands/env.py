...
class EnvironmentAuditor:
    ...
    _ALLOW_LIST: Tuple[str, ...] = (
        ...
        "CEREBRO_TEMP",
        "CEREBRO_DEBUG",
        "CEREBRO_DISABLE_USAGE_TRACKING",
        "CTF_PROVIDER",
    )
    ...
    _SESSION_DIR_NAME = ".cai/session_env"
    ...
    def set_session_value(self, name: str, value: str) -> Tuple[bool, str]:
        ...
        ``~/.cai/session_env/<workspace_hash>.json`` only.
        """
        key = name.strip().upper()
        if not key:
    ...
    def run_audit(self) -> List[AuditCheck]:
        ...
        # the REPL against local LLM backends configured via `CEREBRO_MODEL` or
        ...
        if not has_any_provider_key:
            cai_model = os.environ.get("CEREBRO_MODEL", "").strip()
            if cai_model:
                local_indicators = (
                    "ollama",
                    "litellm",
            ...
            local_envs = ("LITELLM_SERVER", "OLLAMA_URL", "LLM_LOCAL", "CEREBRO_LOCAL_MODEL")
            ...
            # Historically users had to set CEREBRO_ALLOW_LOCAL=1 to opt-in; make
            # the opt-in implicit to support local LLM development workflows.
            allow_local = os.environ.get("CEREBRO_ALLOW_LOCAL", "1").strip().lower()
    ...
    def report_dir(self) -> Path:
        return self._workspace_root / ".cai" / "reports"
