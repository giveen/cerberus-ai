...
from cai.repl.commands.config import CONFIG_STORE, _is_secret, _mask
from cai.repl.commands.cost import CostCommand, USAGE_TRACKER
from cai.repl.commands.env import ENV_AUDITOR
from cai.sdk.agents.agent_registry import AGENT_REGISTRY
from cai.sdk.agents.items import ItemHelpers, MessageOutputItem, ReasoningItem, ToolCallItem, ToolCallOutputItem
from cai.sdk.agents.result import RunResultStreaming
from cai.sdk.agents.run import DEFAULT_MAX_TURNS, RunConfig, Runner
from cai.sdk.agents.simple_agent_manager import AGENT_MANAGER
from cai.sdk.agents.stream_events import AgentUpdatedStreamEvent, RunItemStreamEvent
...
class ExecutionSupervisor:
    ...
    async def run(self, *, agent: Any, options: RunOptions) -> ExecutionImpactSummary:
        ...
        journal_dir = self._workspace_root / ".cai" / "session" / "runs"
    ...
    def _debug_stream_enabled(self) -> bool:
        debug_raw = os.getenv("CEREBRO_DEBUG", "0").strip().lower()
        verbose_raw = os.getenv("CEREBRO_VERBOSE", "0").strip().lower()
    ...
    def _ensure_registry_entry(self, agent: Any, requested_agent_name: Optional[str]) -> Optional[str]:
        ...
        model_name = getattr(getattr(agent, "model", None), "model", os.getenv("CEREBRO_MODEL", "unknown"))
        ...
        agent_type = requested_agent_name or os.getenv("CEREBRO_AGENT_TYPE", display_name.lower().replace(" ", "_"))
...
class RunCommand(FrameworkCommand):
    ...
    def help(self) -> str:
        ...
        return (
            ...
            f"Default max turns: {default_turns}. If CEREBRO_MAX_TURNS is unset or unbounded, the supervisor enforces {default_turns} turns for safety.\n"
    ...
    def _resolve_agent(self, agent_name: Optional[str]) -> Any:
        ...
        requested_agent = agent_name or os.getenv("CEREBRO_AGENT_TYPE", "one_tool")
        try:
