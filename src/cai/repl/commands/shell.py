...
from cai.repl.commands.config import _is_secret, _mask
from cai.tools.workspace import get_project_space

console = Console()

_DEFAULT_TIMEOUT_SECONDS = max(30, int(os.getenv("CEREBRO_SHELL_TIMEOUT_SECONDS", os.getenv("CEREBRO_COMMAND_TIMEOUT_SECONDS", "600"))))
...
class SecureSubprocess:
    ...
    def build_clean_environment(self) -> tuple[Dict[str, str], Dict[str, str]]:
        ...
        for key, value in source_env.items():
            ...
            if key in allowed_core or key.startswith("LC_") or key.startswith("CAI_"):
                clean_env[key] = value
