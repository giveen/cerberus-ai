...
class PriceTable:
    ...
    def _load_pricing_json(self) -> None:
        ...
        candidates = [
            ...
            Path(os.environ.get("CEREBRO_PRICING_JSON", "")) if os.environ.get("CEREBRO_PRICING_JSON") else None,
        ]
...
class CostCommand(Command):
    ...
    def _get_global_summary(self) -> str:
        if not getattr(GLOBAL_USAGE_TRACKER, "enabled", True):
            return (
                ...
                "Set CEREBRO_DISABLE_USAGE_TRACKING=false to enable it."
            )
    ...
    def handle_reset(self, args: Optional[List[str]] = None) -> bool:
        ...
        usage_file = Path(Path.home()) / ".cai" / "usage.json"
