...
from cai.repl.ui.logging import prepare_prompt_history_file
...
class CerebroPromptHUD:
    ...
    def _resolve_workspace_name(self) -> str:
        val = self._cfg_get("CEREBRO_WORKSPACE")
        if val:
            return val
        try:
    ...
    def _resolve_target_name(self) -> str:
        return self._cfg_get("CEREBRO_TARGET") or self._cfg_get("CTF_IP") or ""
    ...
    def _resolve_cost_limit(self) -> float:
        raw = self._cfg_get("CEREBRO_PRICE_LIMIT")
        if not raw:
            return 0.0
        try:
            return float(raw)
    ...
    def _resolve_model_name(self) -> str:
        if self._model_provider is not None:
            try:
                ...
                pass
        return self._cfg_get("CEREBRO_MODEL") or "cerebro1"
    ...
    def _resolve_agent_status(self) -> str:
        if self._agent_status_provider is not None:
            try:
                ...
                pass
        state = self._cfg_get("CEREBRO_AGENT_STATUS")
        if state:
            return state
        return "Thinking" if self._cfg_get("CEREBRO_STREAM", "false").lower() == "true" else "Idle"
...
def get_user_input(
    ...
    current_text,
    *,
    hud: Optional[CerebroPromptHUD] = None,
    config: Any = None,
    workspace_manager: Any = None,
    cost_manager: Any = None,
    model_provider: Optional[Callable[[], str]] = None,
    ...
    prompt_hud = hud or CerebroPromptHUD(
        config=config,
