...
from cai.repl.commands import FuzzyCommandCompleter, get_command, handle_command
...
class CerebroKeyRegistry:
    ...
    def _load_binding_overrides(self) -> Dict[str, Any]:
        raw = CONFIG_STORE.get("CEREBRO_KEYBINDINGS")
        if not raw or raw == "Not set":
            raw = os.getenv("CEREBRO_KEYBINDINGS")
        if not raw:
            return {}
        ...
        try:
            ...
            self._key_log("Key Log: invalid CEREBRO_KEYBINDINGS payload; using defaults")
            return {}
    ...
    async def _cycle_workspace(self, _event: KeyPressEvent) -> None:
        ...
        current = os.getenv("CEREBRO_WORKSPACE", "")
    ...
    async def _cycle_model(self, _event: KeyPressEvent) -> None:
        ...
        current = os.getenv("CEREBRO_MODEL", "")
    ...
    def _discover_models(self) -> List[str]:
        custom = CONFIG_STORE.get("CEREBRO_MODEL_CYCLE") or os.getenv("CEREBRO_MODEL_CYCLE", "")
    ...
    def _path_completion(text: str) -> Optional[str]:
        left, token = CerebroKeyRegistry._split_token(text)
