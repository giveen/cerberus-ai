...
class CerebroToolbar:
    ...
    def _resolve_cost_status(self) -> Tuple[float, float]:
        ...
        try:
            if self._cost_manager:
                ...
                limit = float(os.getenv("CEREBRO_PRICE_LIMIT", "0"))
                return (total, limit)
        except Exception:
            pass
        return (0.0, 0.0)
    ...
    def _resolve_workspace(self) -> str:
        ...
        try:
            ...
            pass
        return os.getenv("CEREBRO_WORKSPACE", "default")

    def _resolve_model(self) -> str:
        """Get active model name."""
        model = os.getenv("CEREBRO_MODEL", "gpt-4o-mini")
        if len(model) > 16:
            return model[:14] + "…"
        return model
    ...
    def _resolve_target(self) -> str:
        ...
        return os.getenv("CEREBRO_TARGET", "")

    def _resolve_privacy_mode(self) -> bool:
        """Check if privacy/screenshare mode is enabled."""
        return os.getenv("CEREBRO_SCREENSHARE", "false").lower() == "true"
