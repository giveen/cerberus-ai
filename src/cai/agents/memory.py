...
--------------------------------------------------------
   ...
   CEREBRO_MEMORY: Enables the use of memory functionality in CAI
    can adopt values:
    - episodic: for episodic memory store
    - semantic: for semantic memory store
    - all: for all memory stores
   CEREBRO_MEMORY_COLLECTION: Name of the collection in Qdrant
    (required if CEREBRO_MEMORY=episodic)
   CEREBRO_MEMORY_ONLINE: Enables online learning (incremental updates)
   CEREBRO_MEMORY_OFFLINE: Trigger offline learning (@2_jsonl_to_memory.py) when
