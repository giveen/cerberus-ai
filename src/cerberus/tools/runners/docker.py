...
try:
    ...
    get_cerberus_logger = None  # type: ignore[assignment]
...
_DEFAULT_NETWORK = "cerberus-internal-no-egress"
...
_OUTPUT_CAPTURE_LIMIT_CHARS = max(10_000, int(os.getenv("CERBERUS_TOOL_OUTPUT_CAPTURE_LIMIT_CHARS", "120000")))
_OUTPUT_HEAD_CHARS = max(1_000, int(os.getenv("CERBERUS_TOOL_OUTPUT_HEAD_CHARS", "8000")))
_OUTPUT_TAIL_CHARS = max(1_000, int(os.getenv("CERBERUS_TOOL_OUTPUT_TAIL_CHARS", "8000")))
...
class CerebroDockerTool:
    ...
    def __init__(self) -> None:
        ...
        self._audit_log = (self._workspace / ".cerberus" / "audit" / "docker_gateway.jsonl").resolve()
        self._logger = get_cerberus_logger() if get_cerberus_logger is not None else None
        ...
        self._network_name = self._config_get("CERBERUS_DOCKER_INTERNAL_NETWORK", _DEFAULT_NETWORK) or _DEFAULT_NETWORK
    ...
    def create_container(
        ...
        *,
        ...
        labels = {
            "cerberus.gateway": "1",
            "cerberus.gateway.managed": "1",
            "cerberus.gateway.created_at": datetime.now(tz=UTC).isoformat(),
            "cerberus.gateway.ttl": str(ttl_value),
            "cerberus.gateway.internet_access": "1" if internet_access else "0",
        }
        create_kwargs = {
            "image": normalized,
    ...
    def _create_for_compat(self, *, args: Any) -> Dict[str, Any]:
        ...
        image = str(payload.get("image") or self._config_get("CERBERUS_DOCKER_DEFAULT_IMAGE", "alpine:latest"))
    ...
    def _ensure_internal_network(self, client: Any) -> str:
        ...
        try:
            network = client.networks.create(
                ...
                internal=True,
                attachable=False,
                labels={"cerberus.gateway": "1", "cerberus.gateway.network": "internal"},
            )
    ...
    async def _exec_async_stream(
        ...
        *,
        ...
        if tool_name:
            with suppress(Exception):
                from cerberus.tools.agent_info import _get_agent_token_info
                from cerberus.util import start_tool_streaming, update_tool_streaming, finish_tool_streaming
    ...
    def _ttl_seconds(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CERBERUS_DOCKER_TTL_SECONDS", _DEFAULT_TTL_SECONDS)
        return max(30, int(raw or _DEFAULT_TTL_SECONDS))

    def _mem_limit_mb(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CERBERUS_DOCKER_MEM_LIMIT_MB", _DEFAULT_MEM_LIMIT_MB)
        return max(64, min(_MAX_MEM_LIMIT_MB, int(raw or _DEFAULT_MEM_LIMIT_MB)))

    def _nano_cpus(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CERBERUS_DOCKER_NANO_CPUS", _DEFAULT_NANO_CPUS)
        return max(100_000_000, min(_MAX_NANO_CPUS, int(raw or _DEFAULT_NANO_CPUS)))

    def _safe_images(self) -> List[str]:
        raw = self._config_get("CERBERUS_SAFE_DOCKER_IMAGES") or self._config_get("CERBERUS_DOCKER_SAFE_IMAGES")
...
DOCKER_TOOL = LazyToolProxy(CerebroDockerTool)
