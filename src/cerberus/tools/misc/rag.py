...
class CerebroRAGTool:
    ...
    def __init__(self) -> None:
        ...
        self._root = (self._workspace / ".cerberus" / "rag_engine").resolve()
    ...
    def _choose_embedding_provider(self) -> str:
        ...
        requested = os.getenv("CERBERUS_RAG_EMBEDDINGS_PROVIDER", "").strip().lower()
        ...
        if os.getenv("OPENAI_API_KEY", "").strip():
            return "openai"
        if os.getenv("CERBERUS_OLLAMA_URL", "").strip() or os.getenv("OLLAMA_HOST", "").strip():
            return "ollama"
        return "local"

    def _is_privacy_mode(self) -> bool:
        env_flag = os.getenv("CERBERUS_PRIVACY_MODE", "").strip().lower()
        if env_flag in {"1", "true", "yes", "on"}:
            return True
        try:
            value, _tier = CONFIG_STORE.resolve("CERBERUS_PRIVACY_MODE")
    ...
    async def _embed_openai(self, text: str) -> Optional[List[float]]:
        ...
        endpoint = os.getenv(
            ...
            os.getenv("CERBERUS_API_BASE", "http://localhost:8000/v1").rstrip("/") + "/embeddings",
        )
        model = os.getenv("CERBERUS_RAG_OPENAI_EMBED_MODEL", "text-embedding-3-small")
    ...
    async def _embed_ollama(self, text: str) -> Optional[List[float]]:
        base = os.getenv("CERBERUS_OLLAMA_URL", "").strip() or os.getenv("OLLAMA_HOST", "").strip() or "http://127.0.0.1:11434"
        endpoint = base.rstrip("/") + "/api/embeddings"
        model = os.getenv("CERBERUS_RAG_OLLAMA_MODEL", "bge-small")
    ...
    async def _embed_hf_local(self, text: str) -> Optional[List[float]]:
        ...
        try:
            ...
            model_name = os.getenv("CERBERUS_RAG_HF_MODEL", "BAAI/bge-small-en-v1.5")
...
RAG_TOOL = LazyToolProxy(CerebroRAGTool)
