...
class CerebroCryptoTool:
    ...
    def __init__(self) -> None:
        ...
        self._audit_path = (self._workspace / ".cerberus" / "audit" / "crypto_jobs.jsonl").resolve()
    ...
    async def _prepare_hash_input(self, *, hash_value: str, input_file: str) -> Optional[Path]:
        ...
        if not hv:
            return None
        target = (self._workspace / ".cerberus" / "tmp" / f"hash_{secrets.token_hex(6)}.txt").resolve()
    ...
    def _encrypt_cleartext(self, cleartext: str) -> str:
        master = os.getenv("CERBERUS_PROJECT_MASTER_KEY", "") or os.getenv("CERBERUS_MASTER_KEY", "")
        if not master:
