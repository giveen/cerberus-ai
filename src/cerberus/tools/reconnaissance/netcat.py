...
class CerebroNetcatTool:
    ...
    def __init__(self) -> None:
        ...
        self._audit_path = (self._workspace / ".cerberus" / "audit" / "netcat_connections.jsonl").resolve()
        ...
        with suppress(Exception):
            ...
            for item in patterns:
                if "/" in item:
                    ...
                    return True, ""
            return False, f"target {target} not present in CERBERUS_ENGAGEMENT_SCOPE/CERBERUS_ALLOWED_TARGETS"
        for item in patterns:
            norm = item.lower()
    ...
    def _allowed_ports() -> set[int]:
        raw = os.getenv("CERBERUS_ALLOWED_PORTS", "").strip()
        if not raw:
...
NETCAT_TOOL = LazyToolProxy(CerebroNetcatTool)
