"""Cerberus Protocol Intelligence Engine (CPIE).
...
from cerberus.internal.components.schema import (
    CerberusFinding,
    ExecutionTelemetry,
    ToolRequest,
    ToolResult,
...
from cerberus.memory.storage import CerberusStorageHandler, EvidenceRecord
...
def _load_config_factory() -> Callable[[], Any]:
    ...
    _CONFIG_SPEC = importlib.util.spec_from_file_location("cerberus.util.config", _CONFIG_MODULE_PATH)
    if _CONFIG_SPEC is None or _CONFIG_SPEC.loader is None:  # pragma: no cover
        raise ImportError(f"Unable to load Cerberus config module from {_CONFIG_MODULE_PATH}")
    _CONFIG_MODULE = importlib.util.module_from_spec(_CONFIG_SPEC)
    sys.modules.setdefault("cerberus.util.config", _CONFIG_MODULE)
    _CONFIG_SPEC.loader.exec_module(_CONFIG_MODULE)
    _CONFIG_MODULE.CerberusConfig.model_rebuild()
    return cast(Callable[[], Any], _CONFIG_MODULE.get_cerberus_config)


get_cerberus_config = _load_config_factory()


_CPIE_LOGGER = logging.getLogger("cerberus.cpie")
...
class CerberusProtocolEngine:
    ...
    def __init__(
        ...
        *,
        ...
        memory_bus: Optional[CerberusMemoryBus] = None,
        storage_handler: Optional[CerberusStorageHandler] = None,
        max_concurrency: Optional[int] = None,
        connect_timeout: float = _DEFAULT_CONNECT_TIMEOUT,
        read_timeout: float = _DEFAULT_READ_TIMEOUT,
    ) -> None:
        self.config = config or get_cerberus_config()
        ...
        self.memory_bus = memory_bus or CerberusMemoryBus.get_instance(workspace_root=str(self.workspace_root))
        self.storage_handler = storage_handler or CerberusStorageHandler.get_instance(workspace_root=str(self.workspace_root))
    ...
    def _derive_max_concurrency(self) -> int:
        configured = os.getenv("CERBERUS_NETWORK_MAX_CONCURRENCY")
        if configured:
            try:
    ...
    def _pathguard_audit(_event: str, _payload: Any) -> None:
        return
__all__ = ["CerberusProtocolEngine"]
