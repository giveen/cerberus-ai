...
_CPTE_LOGGER = logging.getLogger("cerberus.cpte")
...
class CerberusMetricsEngine:
    ...
    async def __aenter__(self) -> "CerberusMetricsEngine":
        await self.start()
        return self
...
async def process_metrics(path: str, sid: Optional[str] = None) -> bool:
    ...
    engine = CerberusMetricsEngine(session_id=sid)
    try:
