...
- Emits basic metrics via `cerberus.rag.metrics`
...
def get_ingestor(adapter: Any, **kwargs) -> IngestionManager:
    ...
    if key not in _INGESTORS:
        ...
        env_batch_size = int(os.getenv("CERBERUS_RAG_BATCH_SIZE", "50"))
        env_batch_interval = float(os.getenv("CERBERUS_RAG_BATCH_INTERVAL", "1.0"))
        env_max_retries = int(os.getenv("CERBERUS_RAG_MAX_RETRIES", "3"))
        env_backoff_base = float(os.getenv("CERBERUS_RAG_BACKOFF_BASE", "0.2"))
        env_backoff_factor = float(os.getenv("CERBERUS_RAG_BACKOFF_FACTOR", "2.0"))
        env_ttl = os.getenv("CERBERUS_RAG_TTL_SECONDS")
        env_retention = int(os.getenv("CERBERUS_RAG_RETENTION_INTERVAL", "3600"))
...
class PathGuardIngestionManager:
    ...
    def _get_pathguard(self):
        if self._pg is None:
            try:
                from cerberus.tools.reconnaissance.filesystem import PathGuard  # type: ignore
