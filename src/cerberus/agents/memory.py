...
--------------------------------------------------------
    ...
    CERBERUS_MEMORY: Enables the use of memory functionality in Cerberus
    can adopt values:
    - episodic: for episodic memory store
    - semantic: for semantic memory store
    - all: for all memory stores
   CERBERUS_MEMORY_COLLECTION: Name of the collection in Qdrant
    (required if CERBERUS_MEMORY=episodic)
   CERBERUS_MEMORY_ONLINE: Enables online learning (incremental updates)
   CERBERUS_MEMORY_OFFLINE: Trigger offline learning (@2_jsonl_to_memory.py) when
    cerberus.client.run() finishes
"""
...
def get_previous_memory(query: str) -> str:
    """Query the vector DB for previous memory context."""
    try:
        from cerberus.rag.vector_db_adapter import get_vector_db_adapter
        adapter = get_vector_db_adapter()
        results = adapter.search(collection_name="_all_", query_text=query or "", limit=3)
