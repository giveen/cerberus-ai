"""
Cerberus agents abstraction layer

Cerberus abstracts its cybersecurity behavior via agents and agentic patterns.
...
# even when Cerberus is installed from PyPI
try:
...
try:
    from cerberus.agents.flag_discriminator import flag_discriminator, transfer_to_flag_discriminator
...
def get_available_agents() -> Dict[str, Agent]:  # pylint: disable=R0912  # noqa
    ...
    from the cerberus/agents folder.
    ...
    try:
        from cerberus.agents.patterns import PATTERNS
...
def get_agent_by_name(agent_name: str, custom_name: str = None, model_override: str = None, agent_id: str = None) -> Agent:
    ...
    from cerberus.agents.factory import get_agent_factory
    ...
    if hasattr(agent, "model") and hasattr(agent.model, "__class__"):
        try:
            ...
            if model_class.__name__ == "OpenAIChatCompletionsModel":
                ...
                try:
                    from cerberus.repl.commands.mcp import get_mcp_tools_for_agent
