<%
    import os
    from cerberus.util import cli_print_tool_call
    try:
        from cerberus.rag.vector_db import get_previous_memory
    except Exception as e:
        # Silently ignore if RAG module is not available
        pass
    from cerberus import is_caiextensions_memory_available
    
    # Import compact summary function
    try:
        from cerberus.repl.commands.memory import get_compacted_summary
    ...
    env_context = os.getenv('CERBERUS_ENV_CONTEXT', 'true').lower()
    # Get memory from vector db if RAG is enabled
    rag_enabled = os.getenv("CERBERUS_MEMORY", "?").lower() in ["episodic", "semantic", "all"]
    memory = ""
    if rag_enabled:
        if os.getenv("CERBERUS_MEMORY", "?").lower() in ["semantic", "all"]:
