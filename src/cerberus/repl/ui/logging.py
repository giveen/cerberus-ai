...
- CerberusLogger: semantic, security-first logger with terminal + disk + buffer sinks
...
_HISTORY_FILE = Path.home() / ".cerberus_history"
...
class ForensicHandler:
    ...
    def __init__(
        ...
        *,
        ...
        self._worker = threading.Thread(target=self._writer_loop, name="cerberus-forensic-writer", daemon=True)
        self._worker.start()
    ...
    def _log_path_for_workspace(self, workspace: Path) -> Path:
        return workspace / ".cerberus" / "audit" / f"forensic_{self._session_id}.jsonl"
...
class CerberusLogger:
    ...
    def _build_terminal_logger(console: Console, session_id: str) -> Logger:
        logger_name = f"cerberus.ui.{session_id}"
    ...
    def _env_show_thought() -> bool:
        value = str(os.environ.get("CERBERUS_THINK", os.environ.get("CERBERUS_LOG_THOUGHTS", "false"))).strip().lower()
...
_GLOBAL_LOGGER: Optional[CerberusLogger] = None
...
def get_cerberus_logger() -> CerberusLogger:
    global _GLOBAL_LOGGER
    if _GLOBAL_LOGGER is None:
        _GLOBAL_LOGGER = CerberusLogger()
...
def setup_session_logging() -> Path:
    ...
    logger.audit(
        ...
        data={
            ...
            "workspace": str(CerberusLogger._workspace_root()),
