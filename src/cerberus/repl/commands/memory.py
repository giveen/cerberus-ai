"""
Memory command for Cerebro REPL.
Manages memory storage in .cerberus/memory for persistent context.
"""
...
def get_compact_model():
    try:
        from cerberus.repl.commands.compact import get_compact_model as _get_compact_model
...
try:
    ...
    MEMORY_DIR = Path.home() / ".cerberus" / "memory"
                ...
                for config in PARALLEL_CONFIGS:
                    if config.id and config.id.upper() == agent_id:
                        ...
                        if config.agent_name.endswith("_pattern"):
                            ...
                            from cerberus.agents.patterns import get_pattern
...
Model: {get_compact_model() or os.environ.get("CERBERUS_MODEL", "gpt-4")}
...
"""
            ...
            os.environ['CERBERUS_MEMORY'] = 'true'
...
"""
    ...
    async def _ai_summarize_history(self, agent_name: Optional[str] = None) -> Optional[str]:
        ...
        from cerberus.repl.commands.compact import get_compact_model, get_custom_prompt
        
        # Create summary agent
        model_name = get_compact_model() or os.environ.get("CERBERUS_MODEL", "cerebro1")
