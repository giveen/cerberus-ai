...
Manages three tiers of settings for the Cerebro framework:
  ...
  2. **User config**    — persistent YAML at ``~/.cerberus/config.yaml``, loaded
...
Encrypted storage (optional):
    ...
    kept at ``~/.cerberus/.keyring`` (mode 0600).  The logic is exposed via
...
Registration::
    ...
    from cerberus.repl.commands.config import CONFIG_COMMAND_INSTANCE

Usage::

    /config                   → show all (masked)
    /config show              → same
    /config get CERBERUS_MODEL     → resolved value + tier source
    /config set CERBERUS_MODEL gpt-4o
    /config set CERBERUS_TEMP 0.7  → validated against schema
    /config reset CERBERUS_MODEL   → remove session + user override
    /config source CERBERUS_MODEL  → show which tier supplies the value
...
_SYSTEM_ENTRIES: List[ConfigEntry] = [
    ...
    ConfigEntry(name="CTF_INSIDE",   description="Conquer CTF from inside container",   default="true",
                value_type="bool"),
    # Core Cerebro
    ConfigEntry(name="CERBERUS_MODEL",        description="Default model for all agents",      default="cerebro1"),
    ConfigEntry(name="CERBERUS_DEBUG",        description="Debug level (0=off,1=verbose,2=cli)", default="1",
                value_type="int", min_val=0, max_val=2),
    ConfigEntry(name="CERBERUS_BRIEF",        description="Enable brief output mode",           default="false",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_MAX_TURNS",    description="Maximum agent turns (inf = unlimited)", default="inf"),
    ConfigEntry(name="CERBERUS_TRACING",      description="Enable OpenTelemetry tracing",       default="true",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_AGENT_TYPE",   description="Default agent type",                 default="one_tool"),
    ConfigEntry(name="CERBERUS_STATE",        description="Enable stateful mode",               default="false",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_MEMORY",       description="Memory mode (false/episodic/semantic/all)", default="false",
                allowed_values=["false", "episodic", "semantic", "all", "False"]),
    ConfigEntry(name="CERBERUS_MEMORY_ONLINE",         description="Enable online memory",  default="false",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_MEMORY_OFFLINE",        description="Enable offline memory", default="false",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_MEMORY_ONLINE_INTERVAL",description="Turns between online memory updates", default="5",
                value_type="int", min_val=1),
    ConfigEntry(name="CERBERUS_ENV_CONTEXT",  description="Inject dirs/env into LLM context",  default="true",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_PRICE_LIMIT",  description="Session price limit in USD",         default="1",
                value_type="float", min_val=0.0),
    ConfigEntry(name="CERBERUS_REPORT",       description="Reporter mode",                      default="ctf",
                allowed_values=["ctf", "nis2", "pentesting", "false"]),
    ConfigEntry(name="CERBERUS_SUPPORT_MODEL",    description="Model for support agent",         default="o3-mini"),
    ConfigEntry(name="CERBERUS_SUPPORT_INTERVAL", description="Turns between support agent runs",default="5",
                value_type="int", min_val=1),
    ConfigEntry(name="CERBERUS_STREAM",       description="Enable streaming responses",          default="true",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_WORKSPACE",    description="Current workspace name",              default=None),
    ConfigEntry(name="CERBERUS_WORKSPACE_DIR",description="Path to workspace directory",         default=None,
                value_type="path"),
    ConfigEntry(name="CERBERUS_GUARDRAILS",   description="Enable prompt injection guardrails",  default="true",
                value_type="bool"),
    ConfigEntry(name="CERBERUS_PARALLEL",     description="Number of parallel agent instances",  default="1",
                value_type="int", min_val=1),
    ConfigEntry(name="CERBERUS_TEMP",         description="Default LLM temperature",             default="1.0",
...
class ConfigurationStore:
    ...
    Resolution order (highest priority first):
      session → env (existing os.environ) → user (~/.cerberus/config.yaml) → system defaults

    The store is process-global; obtain it via ``CONFIG_STORE``.
    """

    _USER_CONFIG_PATH = Path.home() / ".cerberus" / "config.yaml"
    _KEY_PATH         = Path.home() / ".cerberus" / ".keyring"
    ...
    def set(self, name: str, value: str, *, tier: str = TIER_SESSION) -> Tuple[bool, str]:
        ...
        - ``tier=user``    — persistent (written to ``~/.cerberus/config.yaml``).
    ...
    def _notify_registry(name: str, value: str) -> None:
        ...
        try:
            from cerberus.repl.commands.agent import AgentRegistry  # type: ignore[import-untyped]
...
class ConfigCommand(Command):
    ...
    def help(self) -> str:
        return (
            ...
            "WARNING: '--persist' writes the value to ~/.cerberus/config.yaml."
        )
    ...
    async def execute(self, args: List[str]) -> bool:
        ...
        # Fallback: treat as a get by name (convenience: /config CERBERUS_MODEL)
        if args[0].isupper() or args[0].startswith("CERBERUS_") or args[0].startswith("CTF_"):
