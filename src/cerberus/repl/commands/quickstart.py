...
from cerberus.repl.commands.base import FrameworkCommand, get_command, register_command
from cerberus.tools.workspace import ProjectSpace
...
class OnboardingManager:
    ...
    async def _configure_workspace(
        ...
        await self._invoke_command("config", ["set", "CERBERUS_WORKSPACE_DIR", str(workspace_dir)])
        await self._invoke_command("/workspace", ["set", project_name])
        await self._invoke_command("config", ["set", "CERBERUS_WORKSPACE", project_name, "--persist"])
        await self._invoke_command("config", ["set", "CERBERUS_WORKSPACE_DIR", str(workspace_dir), "--persist"])
    ...
    async def _configure_provider(
        ...
        await self._invoke_command("config", ["set", "CERBERUS_MODEL", model, "--persist"])
    ...
    async def _select_template(
        ...
        await self._invoke_command("config", ["set", "CERBERUS_AGENT_TYPE", template.primary_agent, "--persist"])
    ...
    def _new_command(self, command_name: str) -> Optional[FrameworkCommand]:
        if command_name == "config":
            from cerberus.repl.commands.config import ConfigCommand

            return ConfigCommand()
        if command_name == "/workspace":
            from cerberus.repl.commands.workspace import WorkspaceCommand

            return WorkspaceCommand()
        if command_name == "env":
            from cerberus.repl.commands import env as env_module

            env_module.ENV_AUDITOR = env_module.EnvironmentAuditor()
            return env_module.EnvCommand()
        if command_name == "/model":
            from cerberus.repl.commands import model as model_module

            model_module._GLOBAL_ORCHESTRATOR = None
            return model_module.ModelCommand()
        if command_name == "/platform":
            from cerberus.repl.commands import platform as platform_module
    ...
    def _reset_workspace_scoped_caches(self) -> None:
        from cerberus.tools import workspace as workspace_module
    ...
    def _state_path(self) -> Path:
        ...
        return project_space.get_path(".cerberus", "session", "quickstart_state.json", create_parent=True)

    def _brief_path(self) -> Path:
        project_space = ProjectSpace.from_environment()
        project_space.initialize()
        return project_space.get_path(".cerberus", "session", "engagement_bootstrap.md", create_parent=True)
