...
from cerberus.repl.commands.base import FrameworkCommand, handle_command as _base_handle_command, register_command

console = Console()

_CASE_NAME_RE = re.compile(r"^[A-Za-z0-9_-]{3,64}$")
_AUDIT_MAGIC = "CERBERUS_AUDIT_V1"
...
class CaseManager:
    ...
    def __init__(self, *, user: str, memory: MemoryManager) -> None:
        self._user = user
        self._memory = memory
        self._base = Path(os.getenv("CERBERUS_WORKSPACE_DIR", str(Path.cwd() / "workspaces"))).expanduser().resolve()
        self._cases_root = self._base / "cases"
        self._meta_root = self._base / ".cerberus"
    ...
    def archive_case(self, case_name_or_id: str, passphrase: str) -> Path:
        ...
        with tempfile.NamedTemporaryFile(prefix="cerberus_case_", suffix=".tar.gz", delete=False) as tmp:
    ...
    def _activate_case(self, case: CaseMetadata) -> None:
        ...
        os.environ["CERBERUS_WORKSPACE"] = case.name
        os.environ["CERBERUS_WORKSPACE_DIR"] = str(self._base)
        os.environ["CERBERUS_WORKSPACE_ACTIVE_ROOT"] = str(root)
        os.environ["WORKSPACE_ROOT"] = str(root)
        os.environ["CERBERUS_SHARED_ROOT"] = str(case.shared_dir)
        os.environ["CERBERUS_PRIVATE_ROOT"] = str(case.private_dir)
        os.environ["CERBERUS_AGENT_WRITE_ROOT"] = str(case.private_dir)
    ...
    def _install_command_hook(self) -> None:
        ...
        import cerberus.repl.commands.base as base_module
    ...
    def _init_case_dirs(self, root: Path) -> None:
        for rel in ("shared", "private", "logs", "artifacts", "reports", ".cerberus"):
    ...
    def _env_keys() -> List[str]:
        return [
            ...
            "WORKSPACE_ROOT",
            "CERBERUS_SHARED_ROOT",
            "CERBERUS_PRIVATE_ROOT",
            "CERBERUS_AGENT_WRITE_ROOT",
        ]
