...
from cerberus.repl.commands.shell import SecureSubprocess, SecureSubprocessResult, StreamLine
...
class SandboxManager:
    ...
    def __init__(self, *, workspace_root: Path, memory: MemoryManager, user: str) -> None:
        ...
        self._memory = memory
        self._user = user
        self._state_path = self._workspace_root / ".cerberus" / "session" / _STATE_FILE_NAME
    ...
    async def up(self, options: VirtualizationOptions) -> SandboxState:
        ...
        container_name = f"cerberus-sandbox-{instance_id}"
        state = SandboxState(
    ...
    def _trusted_registries(self) -> List[str]:
        raw = os.getenv("CERBERUS_TRUSTED_REGISTRIES", "").strip()
        if not raw:
            raw = CONFIG_STORE.get("CERBERUS_TRUSTED_REGISTRIES")
    ...
    def _health_policy() -> HealthPolicy:
        try:
            max_cpu = float(os.getenv("CERBERUS_SANDBOX_MAX_CPU_PERCENT", "90"))
        except ValueError:
            max_cpu = 90.0
        try:
            max_mem = float(os.getenv("CERBERUS_SANDBOX_MAX_MEM_MB", "3072"))
        except ValueError:
            max_mem = 3072.0
...
class VirtualizationCommand(FrameworkCommand):
    ...
    def help(self) -> str:
        return (
            ...
            "  - Image registry must be trusted via CERBERUS_TRUSTED_REGISTRIES.\n"
