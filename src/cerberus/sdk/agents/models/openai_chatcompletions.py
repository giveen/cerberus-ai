...
_LLM_TAG_PATTERNS = [
    ...
    _re.compile(r'^```(?:json)?\s*', _re.IGNORECASE | _re.MULTILINE),  # leading markdown fence
    _re.compile(r'\s*```\s*$', _re.MULTILINE),           # trailing markdown fence
]
...
_KEEP_RECENT_CONTEXT_MESSAGES = max(
    2,
    min(4, int(os.getenv("CERBERUS_KEEP_RECENT_MESSAGES", "4") or "4")),
)
...
def _disable_external_trace_export_env() -> None:
    # Local-first runtime: never attempt remote trace exports from model execution.
    for env_key in _EXTERNAL_TRACE_EXPORT_ENV_KEYS:
        os.environ.pop(env_key, None)
    os.environ["CERBERUS_TRACING"] = "false"


def _strip_trace_fields(value: Any) -> Any:
    if isinstance(value, dict):
        cleaned: dict[str, Any] = {}
        for key, nested in value.items():
            if str(key).lower() in _TRACE_METADATA_KEYS:
                continue
            cleaned[key] = _strip_trace_fields(nested)
        return cleaned
    if isinstance(value, list):
        return [_strip_trace_fields(item) for item in value]
    return value
...
def _sanitize_messages_for_llm(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        stripped = _strip_trace_fields(message)
        ...
        if "content" in stripped:
            normalized["content"] = stripped.get("content")

        # Preserve functional chat-completions semantics while dropping metadata.
        if "tool_calls" in stripped:
            normalized["tool_calls"] = _strip_trace_fields(stripped.get("tool_calls"))
        if "tool_call_id" in stripped:
            normalized["tool_call_id"] = stripped.get("tool_call_id")
        if "name" in stripped:
            normalized["name"] = stripped.get("name")
        if "refusal" in stripped:
            normalized["refusal"] = stripped.get("refusal")
        if "cache_control" in stripped:
            normalized["cache_control"] = stripped.get("cache_control")
...
def _messages_debug_view(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        ...
        preview.append({"role": role, "content": str(content or "")})
    return preview
...
def _distill_nmap_output(raw: str) -> str:
    ...
    for line in raw.splitlines():
        match = re.search(r"(?m)^\s*(\d+)/(tcp|udp)\s+open\s+([^\s].*)$", line.strip())
        if not match:
            continue
        port = match.group(1)
        proto = match.group(2)
    ...
    if open_ports:
        preview = ", ".join(open_ports[:20])
        if len(open_ports) > 20:
            preview += f", ... (+{len(open_ports) - 20} more)"
        return (
            f"Nmap summary: Open endpoints detected: {preview}. "
            "Full scan results available in session logs and operator terminal output."
        )

    compact_head = "\n".join(raw.splitlines()[:20]).strip()
    if not compact_head:
        compact_head = "No parsable open-port lines detected."
    return (
        "Nmap summary: No explicit open-port rows matched parser output. "
        "Preview:\n"
        f"{compact_head}\n"
        "Full scan results available in session logs and operator terminal output."
    )
...
def _distill_file_read_output(raw: str) -> str:
...
def _distill_generic_output(raw: str) -> str:
    collapsed = re.sub(r"\n{3,}", "\n\n", raw)
    collapsed = re.sub(r"[ \t]{2,}", " ", collapsed)
    lines = collapsed.splitlines()
    if len(lines) <= 90 and len(collapsed) <= MAX_TOOL_OUTPUT_CHARS:
        return collapsed
    ...
    return (
        ...
        f"{tail}\n"
        "[Distilled for model context; full output preserved in terminal and session logs.]"
    )


def distill_tool_output_for_context(*, tool_name: str, tool_args: Any, output: Any) -> str:
    ...
    is_file_read = (
        any(token in name for token in ("read", "cat", "filesystem", "file"))
        or command_text.startswith("cat ")
        or command_text.startswith("sed -n")
        or command_text.startswith("tail ")
        or command_text.startswith("head ")
    )

    if is_nmap:
        distilled = _distill_nmap_output(raw)
    elif is_file_read:
        distilled = _distill_file_read_output(raw)
    else:
        distilled = _distill_generic_output(raw)

    debug_message = f"Tool output distilled: {len(raw)} -> {len(distilled)} characters."
...
def _build_models_endpoint(target_url: str) -> str:
...
def _emit_preflight_critical_alert(target_url: str) -> None:
    message = (
        f"CRITICAL: Local LLM at {target_url} is unreachable. "
        "Check your network or Ollama/LocalAI status."
    )
    try:
...
async def _run_silent_llm_preflight(*, target_url: str, model: str) -> None:
    reachable, _reason = _check_local_preflight_target(target_url)
    if not reachable:
        if target_url not in _PREFLIGHT_ALERTED_UNREACHABLE:
            _PREFLIGHT_ALERTED_UNREACHABLE.add(target_url)
            _emit_preflight_critical_alert(target_url)
        return

    models_url = _build_models_endpoint(target_url)
    if not models_url:
        return
...
def _schedule_silent_llm_preflight(*, target_url: str, model: str) -> None:
    key = str(target_url or "").strip()
    if not key:
        return

    existing = _PREFLIGHT_BACKGROUND_TASKS.get(key)
    ...
    task = loop.create_task(_run_silent_llm_preflight(target_url=key, model=model))
    _PREFLIGHT_BACKGROUND_TASKS[key] = task

    def _cleanup(_task: asyncio.Task[None], endpoint: str = key) -> None:
        current = _PREFLIGHT_BACKGROUND_TASKS.get(endpoint)
        if current is _task:
            _PREFLIGHT_BACKGROUND_TASKS.pop(endpoint, None)
...
def _sanitize_llm_tool_args(raw: str | None) -> str:
    ...
    Returns a sanitized string without injecting fallback JSON. Callers must
    decide whether an empty result means "no args" or "malformed args".
    """
    if raw is None:
        return ""
    if not raw.strip():
...
def _parse_tool_args(raw: str | None, tool_name: str = "") -> dict[str, Any] | None:
    ...
    Returns a parsed dict when valid JSON is present. Attempts json_repair on
    malformed arguments before giving up, so that recoverable payloads (e.g.
    unescaped quotes) are not silently dropped as empty dicts.
    """
    ...
    try:
        ...
        # Second pass: try json_repair before falling back to lenient parser
        if _json_repair is not None:
            try:
                repaired = _json_repair.loads(cleaned)
                if isinstance(repaired, dict):
                    logger.debug(
                        "Repaired malformed tool arguments for %s via json_repair",
...
def _extract_tool_name_from_error_message(error_text: str) -> str:
    patterns = [
        r"tool\s+['\"]([a-zA-Z0-9_\-]+)['\"]",
...
def _configure_litellm_runtime() -> None:
    """Set safe LiteLLM defaults for local runtime execution.

    The dashboard/runtime uses LiteLLM as a client, not as a proxy service.
    Some LiteLLM versions may still attempt to initialize proxy logging paths
    that depend on optional proxy extras (e.g., orjson), producing noisy
    ImportError traces and failing runs. Keep that path disabled by default.
    """
    enable_message_logging = str(os.getenv("CERBERUS_LITELLM_MESSAGE_LOGGING", "0") or "0").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    # LiteLLM currently imports proxy-only modules while generating
    # cold-storage metadata, even in client-only usage. Patch the helper to
    # fail closed and return None instead of surfacing proxy import errors.
    try:
        ...
        if payload_setup is not None and original is not None and not getattr(original, "_cerberus_safe_wrap", False):
            ...
            def _safe_generate_cold_storage_object_key(*args: Any, **kwargs: Any) -> Any:
                try:
                    return original(*args, **kwargs)
                except Exception:
                    return None

            _safe_generate_cold_storage_object_key._cerberus_safe_wrap = True  # type: ignore[attr-defined]
            payload_setup._generate_cold_storage_object_key = staticmethod(_safe_generate_cold_storage_object_key)
    except Exception:
        # Never fail model initialization due to optional logging internals.
        pass
                    ...
                    if isinstance(msg["content"], str):
                        ...
                        for content_part in msg["content"]:
                            if isinstance(content_part, dict) and isinstance(content_part.get("text"), str):
                                total_len += len(content_part["text"])
...
def _pruning_encoding_for_model(model_name: str):
    if tiktoken is None:
        return None
    ...
    try:
        if (
            ...
            return tiktoken.encoding_for_model(str(model_name))
    except Exception:
        pass

    # Qwen and most OpenAI-compatible local models map well to cl100k.
    try:
...
def _count_single_message_tokens(message: dict[str, Any], *, encoding: Any) -> int:
    if encoding is None:
        return len(json.dumps(message, ensure_ascii=False, default=str)) // 4

    count = 4
    role = message.get("role")
    if isinstance(role, str):
        count += len(encoding.encode(role))
    ...
    # Approximate overhead for tool payloads and names.
    if isinstance(message.get("name"), str):
        count += len(encoding.encode(message["name"]))
    if message.get("tool_calls") is not None:
        count += len(json.dumps(message.get("tool_calls"), ensure_ascii=False, default=str)) // 4
    return count
...
def prune_history(
    ...
    *,
    ...
    keep_recent_messages: int = _KEEP_RECENT_CONTEXT_MESSAGES,
) -> tuple[list[dict[str, Any]], int, int]:
    """Prune old turns while preserving system prompt and recent context.

    Rules:
    - Never remove index 0 (system/session metadata message).
    - Remove oldest messages from index 1 onward when above token budget.
    - Always keep the most recent N (2-4) messages.
    """
    if not messages:
        return messages, 0, 0
    if len(messages) <= 1:
        total, _ = count_tokens_with_tiktoken(messages)
        return messages, 0, int(total)

    keep_recent = max(2, min(4, int(keep_recent_messages or 4)))
    encoding = _pruning_encoding_for_model(model_name)

    token_counts = [_count_single_message_tokens(msg, encoding=encoding) for msg in messages]
    total_tokens = int(sum(token_counts))
    if total_tokens <= max_context_tokens:
        return messages, 0, total_tokens

    protected_start = max(1, len(messages) - keep_recent)
    keep_mask = [True] * len(messages)
    keep_mask[0] = True

    removed_count = 0
    for idx in range(1, protected_start):
        if total_tokens <= max_context_tokens:
            break
        if keep_mask[idx]:
            keep_mask[idx] = False
    ...
    pruned = [msg for idx, msg in enumerate(messages) if keep_mask[idx]]

    # Safety check: preserve top-level system/session metadata message.
    if messages and pruned and pruned[0] is not messages[0]:
        pruned.insert(0, messages[0])
...
class OpenAIChatCompletionsModel(Model):
                ...
                for call_id, output_content in self._converter.tool_outputs.items():
                    ...
                    if not tool_msg_exists:
                        ...
                        tool_msg = {
                            ...
                            "content": distill_tool_output_for_context(
                                tool_name=tool_name,
                                tool_args=tool_args,
    ...
    async def stream_response(
        ...
        try:
            ...
            with generation_span(
                ...
                def _stream_event(event_cls: Any, /, **kwargs: Any) -> Any:
                    nonlocal sequence_number
                    if event_cls is ResponseTextDeltaEvent and "logprobs" not in kwargs:
                        kwargs["logprobs"] = []
                    event = event_cls(sequence_number=sequence_number, **kwargs)
                    sequence_number += 1
                    return event
                ...
                try:
                    ...
                    if _is_tool_arguments_parse_error(e):
                        ...
                        recovered_tool_name = ""
                        recovered_tool_arguments: Any = None

                        # Prefer currently streamed function call metadata if present.
                        if state.function_calls:
                            for existing in reversed(list(state.function_calls.values())):
                                if existing.name:
                                    recovered_tool_name = existing.name
                                    recovered_tool_arguments = existing.arguments
                                    break

                        # Try fallback extraction from COMMITTING_JSON in streamed text.
                        text_candidates: list[str] = []
                        if state.text_content_index_and_output:
                            text_candidates.append(state.text_content_index_and_output[1].text)
                        if ollama_full_content:
                            text_candidates.append(ollama_full_content)
                        extracted = _extract_committing_json_tool_call("\n".join(text_candidates))
                        if extracted is not None:
                            recovered_tool_name, recovered_tool_arguments = extracted

                        # Last attempt: derive from error string itself.
                        if not recovered_tool_name:
                            recovered_tool_name = _extract_tool_name_from_error_message(error_text)

                        if not recovered_tool_name:
                            recovered_tool_name = "malformed_tool_call"
                        ...
                        synthetic_index = max(state.function_calls.keys(), default=-1) + 1
                        state.function_calls[synthetic_index] = ResponseFunctionToolCall(
                            id=FAKE_RESPONSES_ID,
                        ...
                        _RUNTIME_DEBUG_LOGGER.write(
                            channel="trace_debug",
                            message="stream_tool_parse_error_recovered",
                            payload={
                                "tool_name": recovered_tool_name,
                                "exception": error_text[:800],
                            },
                        )
                        logger.warning(
                            "Recovered from streamed tool-argument parse failure via synthetic tool call: %s",
                            recovered_tool_name,
                        )
                        self.suppress_final_output = True
                    else:
    ...
    async def _fetch_response(
        ...
        original_system_message = converted_messages[0] if converted_messages else None
        pruned_messages, removed_count, pruned_token_estimate = prune_history(
            converted_messages,
            model_name=str(self.model),
            max_context_tokens=MAX_CONTEXT_TOKENS,
            keep_recent_messages=_KEEP_RECENT_CONTEXT_MESSAGES,
        )
        converted_messages = pruned_messages

        if removed_count > 0:
            logger.debug(
                "Context Pruned: Removed %s messages to stay under %s tokens.",
                removed_count,
                MAX_CONTEXT_TOKENS,
            )

        # Verification: keep session metadata/system prompt pinned at index 0.
        if original_system_message is not None and converted_messages and converted_messages[0] is not original_system_message:
            converted_messages.insert(0, original_system_message)

        # Strip internal tracing identifiers from outbound LLM payload.
        converted_messages = _sanitize_messages_for_llm(converted_messages)

        # Keep tracing data aligned to the effective message payload after pruning.
        if tracing.include_data():
            span.span_data.input = converted_messages
            ...
            if isinstance(metadata, dict):
                metadata.update(
                    {
                        "context_prune_removed": removed_count,
                        "context_prune_threshold": MAX_CONTEXT_TOKENS,
                        "context_prune_estimated_tokens": int(pruned_token_estimate),
                    }
                )
        ...
        if _debug.DONT_LOG_MODEL_DATA:
            ...
            debug_messages = _messages_debug_view(converted_messages)
            logger.debug(
                f"{json.dumps(debug_messages, indent=2)}\n"
            ...
            # 5) function call output => tool message
                ...
                if model_instance and hasattr(model_instance, "_pending_tool_calls"):
                    ...
                    if call_id in model_instance._pending_tool_calls:
                        ...
                        distilled_output = distill_tool_output_for_context(
                            tool_name=tool_name,
                            tool_args=tool_args,
