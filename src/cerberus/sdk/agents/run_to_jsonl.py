...
def get_session_recorder(workspace_name=None):
    ...
    if os.environ.get("CERBERUS_DISABLE_SESSION_RECORDING", "").lower() == "true":
        return None
...
class DataRecorder:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, workspace_name: str | None = None):
        ...
        # Keep session logs strictly offline-only.
        public_ip = "127.0.0.1"
        ...
        with open(self.filename, 'a', encoding='utf-8') as f:
            session_start = {
                ...
                "cerebro_api_key": os.getenv("CERBERUS_API_KEY", ""),
            }
    ...
    def rec_training_data(self, create_params, msg, total_cost=None, agent_name=None) -> None:
        ...
        Args:
            ...
            total_cost: Optional total accumulated cost from Cerberus instance
        ...
        active_time_seconds = safe_duration_to_float(active_time_str)
        idle_time_seconds = safe_duration_to_float(idle_time_str)
    ...
    def log_session_end(self):
        ...
        try:
            from cerberus.util import get_active_time_seconds, get_idle_time_seconds, COST_TRACKER
            ...
            try:
                ...
                if "cost" in record:
                    if isinstance(record["cost"], dict):
                        ...
                        last_total_cost = safe_duration_to_float(record["cost"])
                if "timing_metrics" in record:
                    if isinstance(record["timing_metrics"], dict):
                        last_active_time = safe_duration_to_float(
                            record["timing_metrics"].get("active_time_seconds", 0.0)
                        )
                        last_idle_time = safe_duration_to_float(
                            record["timing_metrics"].get("idle_time_seconds", 0.0)
                        )
                if "model" in record:
                ...
                if record.get("event") == "session_end":
                    ...
                    if "cost" in record and isinstance(record["cost"], dict):
                        last_total_cost = safe_duration_to_float(
                            record["cost"].get("total_cost", 0.0)
                        )
