...
class MCPUtil:
    """Set of utilities for interop between MCP and Cerberus tools."""

    @staticmethod
    def _serialize_content_item(item: Any) -> dict[str, Any]:
        """Serialize MCP content while preserving legacy output shape."""
    ...
    def to_function_tool(cls, tool: "MCPTool", server: "MCPServer") -> FunctionTool:
        """Convert an MCP tool to a Cerberus function tool."""
    ...
    async def _format_tool_result(cls, result, tool: "MCPTool", server: "MCPServer") -> str:
        ...
        if len(result.content) == 1:
            tool_output = json.dumps(
                cls._serialize_content_item(result.content[0]),
                separators=(",", ":"),
            )
        elif len(result.content) > 1:
            tool_output = json.dumps([cls._serialize_content_item(item) for item in result.content])
        else:
