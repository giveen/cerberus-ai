...
# Use the invoked program name (for example, `cerberus`) for help text so
# examples match what the user actually typed. Fall back to `cerberus`.
APP_COMMAND = Path(sys.argv[0]).name if len(sys.argv) > 0 else "cerberus"
...
def __getattr__(name: str) -> Any:
    ...
    if name == "get_agent_by_name":
        from cerberus.agents import get_agent_by_name as _get_agent_by_name
        ...
        Text(
            "\n".join(
                [
                    ...
                    "  --openai-key        removed (use env/config, e.g. CERBERUS_API_BASE)",
...
def _configure_workspace_env(workspace_flag: Optional[str]) -> None:
    ...
    os.environ["CERBERUS_WORKSPACE_ACTIVE_ROOT"] = str(workspace_root)
    os.environ["WORKSPACE_ROOT"] = str(workspace_root)

    if "CERBERUS_WORKSPACE" not in os.environ:
        os.environ["CERBERUS_WORKSPACE"] = workspace_root.name
    if "CERBERUS_WORKSPACE_DIR" not in os.environ:
        os.environ["CERBERUS_WORKSPACE_DIR"] = str(workspace_root.parent)


def _apply_global_flags(opts: GlobalOptions) -> None:
    if opts.silent:
        os.environ["CERBERUS_SILENT"] = "1"
    if opts.debug:
        os.environ["CERBERUS_DEBUG"] = "2"
        logging.basicConfig(level=logging.DEBUG)
    if opts.verbose:
        os.environ["CERBERUS_VERBOSE"] = "1"
...
def _bootstrap_config_and_env() -> BootstrapState:
    ...
    license_value = os.getenv("CERBERUS_LICENSE_KEY", "").strip()
    if not license_value:
        try:
            cfg_license, _tier = CONFIG_STORE.resolve("CERBERUS_LICENSE_KEY")
...
def _ensure_runtime_prerequisites(state: BootstrapState, command: str) -> None:
    ...
    raise CLIUserError(
        ...
        "or configure CERBERUS_LICENSE_KEY before running interactive workloads."
    )


def _log_support_error(exc: BaseException) -> str:
    support_id = f"SUP-{uuid.uuid4().hex[:12].upper()}"
    workspace_root = Path(os.getenv("CERBERUS_WORKSPACE_ACTIVE_ROOT") or os.getenv("WORKSPACE_ROOT") or os.getcwd()).resolve()
    out_dir = workspace_root / ".cerberus" / "errors"
...
async def _dispatch_repl(opts: GlobalOptions) -> int:
    from cerberus.repl.ui.banner import CerberusBanner
    from cerberus.repl.commands import handle_command as commands_handle_command
    from cerberus.repl.commands.base import SessionContext, set_session
    ...
    if not opts.silent:
        CerberusBanner(console).display()
...
async def _async_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if getattr(args, "openai_key", None) or getattr(args, "export_traces", False):
        raise CLIUserError(
            ...
            "Use CERBERUS_API_BASE and local runtime audit logging instead."
        )
    ...
    # LLM endpoint or CERBERUS_MODEL) are loaded into the process and used by
